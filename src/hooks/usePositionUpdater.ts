...
import { updatePlayerPosition } from '../lib/realtimeDb';

// 위치 업데이트 throttle 시간 (ms) - Firestore 비용 최적화: 500ms
...
export function usePositionUpdater() {
    ...
    const updateMyPosition = useCallback(
        (position: { x: number; y: number; z: number }, rotation: number) => {
            ...
            // 위치 변경 체크 (임계값 0.1m - Firestore 비용 최적화)
            const posChanged =
                !lastPosition.current ||
                Math.abs(lastPosition.current.x - position.x) > 0.1 ||
                Math.abs(lastPosition.current.y - position.y) > 0.1 ||
                Math.abs(lastPosition.current.z - position.z) > 0.1;

            const rotChanged = Math.abs(lastRotation.current - rotation) > 0.1;
